...
T(1,2:Ny-1) = TL;             % all the elements on left wall are kept at a temp of 120deg C
T(2:Nx-1,Ny) = TT;            % all the grid elements on the top wall
T(2:Nx-1,1) = TB;             % all the grid elements on bottom wall
T(Nx,2:Ny-1) = TR;            % all the grid points on right wall

T(1,1) = (TL+TB)/2;           % for the corner points temp definition
T(Nx,1) = (TR+TB)/2;
T(1,Ny) = (TT+TL)/2;
T(Nx,Ny) = (TR+TT)/2;
